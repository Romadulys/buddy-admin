  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [campRes, limRes] = await Promise.allSettled([
        fetch('/api/marketing/social-ads/campaigns?platform=tiktok'),
        fetch('/api/marketing/sea/budgets/limits?scope=tiktok_ads'),
      ])
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      await fetch('/api/marketing/social-ads/campaigns', {
        ...
        body: JSON.stringify({ platform: 'tiktok', name: newName.trim(), objective: newObjective }),
      })
      setNewName('')
    ...
    <div className="p-6 anim-section">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          <h1 className="text-2xl font-bold text-slate-900">🎵 TikTok Ads</h1>
          {!loading && (
            ...
            {budgetLimit?.monthly_limit && (
              <div className="mt-2">
                ...
                <div className="h-2 bg-slate-100 rounded-full overflow-hidden">
                  <div
                    className={`h-full rounded-full transition-all ${(budgetPct ?? 0) >= 90 ? 'bg-red-500' : (budgetPct ?? 0) >= 70 ? 'bg-yellow-500' : 'bg-slate-900'}`}
      ...
      {/* Spark Ads highlight card */}
      <div className="bg-slate-900 text-white rounded-xl p-4 mb-6">
        <div className="flex items-start gap-3">
          <span className="text-2xl flex-shrink-0">⚡</span>
          <div>
            <h3 className="font-semibold mb-1">Spark Ads — Amplification organique</h3>
            <p className="text-sm text-slate-300">
              Les Spark Ads boostent du contenu organique qui marche deja. Plutot que creer du contenu
              pub from scratch, les agents identifient les posts avec le meilleur engagement naturel
              et les amplifient avec du budget paye. Meilleur CPM, meilleur taux de vue, meilleur ROAS.
            </p>
          </div>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-12 text-center text-slate-400">
          {campaigns.length === 0
            ? 'Aucune campagne TikTok. Cliquez sur "Nouvelle campagne TikTok" pour commencer.'
            : 'Aucune campagne correspondant aux filtres.'}
        </div>
      ) : (
        <div className="flex flex-col gap-3">
          {filtered.map((c) => (
            <div key={c.id} className="bg-white rounded-xl border border-slate-200 shadow-sm p-5 hover:border-slate-400 transition-colors">
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-2xl shadow-xl p-6 w-full max-w-md mx-4">
            <h2 className="text-lg font-bold text-slate-900 mb-4">Nouvelle campagne TikTok</h2>
            <form onSubmit={handleCreate} className="flex flex-col gap-4">
              <div>
                ...
                <input
                  ...
                  placeholder="ex: Spark Ads — Top Performers"
